...
# kegg_pathway1(data=c("cid:5997","cid:65094","cid:5280335"),n@org_organism)->da
...
#' diff kegg level metaProfiler
#' 
#' @param n metaProfiler
#' @param level A number of kegg level
#' @return metaProfiler.
#' @examples
...
#' kegg_level(n)
  ...
  kegg_level=function(n,level=2){
    ...
    colnames(get)[1]="name"
    f=get%>%group_by(name)%>%summarise(
      group=group%>%unique() ,  org  =org%>%unique() ,
      Compound_ID=Compound_ID%>%paste(sep = "|",collapse = "|")%>%str_remove_all("NULL\\|")%>%str_remove_all("\\|NULL"),
      Gene_id    =Gene_id%>%paste(sep = "|",collapse = "|")%>%str_remove_all("NULL\\|")%>%str_remove_all("\\|NULL"),
      Protacxns_id =Protacxns_id%>%paste(sep = "|",collapse = "|")%>%str_remove_all("NULL\\|")%>%str_remove_all("\\|NULL"),
      Ecs_id=Ecs_id%>%paste(sep = "|",collapse = "|")%>%str_remove_all("NULL\\|")%>%str_remove_all("\\|NULL")
    )
