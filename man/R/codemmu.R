...
.get_import <- function(enrichKEGG,import_model="degree",kegg_org,Count=1) {
  ...
  all_Count=Count
  d2%>%dplyr::filter(Count>all_Count)->d2
...
#.get_all_org("9606")->data
#org=9606
.get_all_org<-function(org="9606"){
 ...
 cat("download by" ,url)
  ...
  kegg_pathways<-.get_kegg_org(kegg_org)
    ...
    kegg_pathways%>%rbind(datad,.)->kegg_pathways
...
.get_kegg_org<- function(org="hsa") {
  ...
  par=full_join(pathways,x1,by="V2")%>%na.omit%>%full_join( full_join(x,y,by="V2")%>%na.omit%>% dplyr::mutate(name=stringr::str_remove(name,".*:")),by="name")%>%
  ...
  gene=full_join(pathways,x1,by="V2")%>%na.omit%>%full_join( y%>% dplyr::mutate(name=stringr::str_remove(name,".*:")),by="name")%>%
  ...
  b=fread("http://rest.kegg.jp/list/pathway",head=F)%>%inner_join(k%>% dplyr::mutate(V1=stringr::str_remove(V1,".*:")),by = "V1")%>% inner_join(pathways,by="V2")
  ...
  Ecs=fread("http://rest.kegg.jp/list/pathway",head=F)%>%inner_join(k%>% dplyr::mutate(V1=stringr::str_remove(V1,".*:")),by = "V1")%>% inner_join(pathways,by="V2")%>%
...
kegg_pathway1 <- function(data=c("list or data"),org="9606",p_model=c("phyper","fisher"),
                          p.adjust.methods="holm",import_model=c("degree","betweenness"),Count=1) {
  library(dplyr)
  options(dplyr.summarise.inform = FALSE)
  library(data.table)
  if(is.null(org_db)){
    pathways<-get_org(org)
    if(!is.factor(pathways$org)){
  ...
  y <- data %>% purrr::map(function(.x) {
    tryCatch( return( .get_kegg_analyst(name = .x, org = org, kegg_pathways = pathways,
                                        ...
                                        p_model = p_model, p.adjust.methods = p.adjust.methods,Count=Count)), error = function(e) return(NA), finally = print("OK"))
  })
...
kegg_level=function (n, level = 2)
{
    ...
    x <- readLines("https://www.genome.jp/kegg-bin/download_htext?htext=br08901.
keg&format=htext&filedir=")
    s <- x %>% str_detect("!") %>% which()
    a <- x[(s[1] + 1):(s[2] - 1)] %>% str_replace("<b>", "  ") %>%
        str_remove("</b>")
    das <- x[(s[1] + 1):(s[2] - 1)] %>% str_replace("<b>", "  ") %>%
        str_remove("</b>") %>% as.data.frame()
    das$A <- NA
    das$B <- NA
    das$C <- NA
    das$A[str_detect(a, "^A") %>% which()] <- a[str_detect(a,
        "^A") %>% which()] %>% str_remove("A  ")
    das$B[str_detect(a, "^B") %>% which()] <- a[str_detect(a,
        "^B") %>% which()] %>% str_remove("B  ")
    das$C[str_detect(a, "^C") %>% which()] <- a[str_detect(a,
        "^C") %>% which()] %>% str_remove("C  ") %>% str_remove("  [0-9][0-9][0-
9][0-9][0-9]  ")
    s <- das %>% fill(A, .direction = "up") %>% fill(B, .direction = "up") %>%
        na.omit()
    colnames(s)[4] = "name"
    get = left_join(s, org1) %>% na.omit()
    get = get[, -c(1:4)[-c(level + 1)]]
    colnames(get)[1] = "name"
    f = get %>% group_by(name) %>% summarise(group = group %>%
        unique(), org = org %>% unique(), Compound_ID = Compound_ID %>%
        paste(sep = "|", collapse = "|") %>% str_remove_all("NULL\\|") %>%
        str_remove_all("\\|NULL"), Gene_id = Gene_id %>% paste(sep = "|",
        collapse = "|") %>% str_remove_all("NULL\\|") %>% str_remove_all("\\|NUL
L"),
        Protacxns_id = Protacxns_id %>% paste(sep = "|", collapse = "|") %>%
            str_remove_all("NULL\\|") %>% str_remove_all("\\|NULL"),
        Ecs_id = Ecs_id %>% paste(sep = "|", collapse = "|") %>%
            str_remove_all("NULL\\|") %>% str_remove_all("\\|NULL"))
    l = data.frame(ID = f$name, f)
    kegg = kegg_pathway1(data = lapply(n@kegg_analyst[["enrichKEGG"]],
        function(x) {
            x@gene
        }), org_db = l)
    kegg@org = n@org
